...
└──────┬──────────────────────────┘
       ...
       ├─── subtitle-worker (Fargate)──→ 스크립트 기반 SRT  ──→ S3 subtitle.srt
       ├─── render-worker  (Fargate) ──→ Pexels + FFmpeg   ──→ S3 output.mp4
...
| Worker | 위치 | 입력 | 처리 | 출력 |
|---|---|---|---|---|
| **script-worker** | `apps/workers/script` | 토픽, 채널 ID | Gemini 2.5 Flash API 호출 — 뉴스·시사 특화 25~35초 스크립트 | `script.json` → S3 |
| **tts-worker** | `apps/workers/tts` | `script.json` | Edge-TTS `ko-KR-SunHiNeural` | `audio.mp3` → S3 |
| **upload-worker** | `apps/workers/upload` | `output.mp4` | YouTube Data API v3 업로드 (AI 공시 문구, categoryId=25, containsSyntheticMedia) | 업로드 완료, `youtubeVideoId` 저장 |

### Fargate Workers

Fargate는 CPU 집약적인 장시간 처리가 필요한 작업에 사용합니다. ECS Service로 상시 실행하여 Cold Start를 방지하고, 자체 SQS Long Polling 루프로 메시지를 수신합니다([ADR 009](../adr/009-fargate-sqs-long-polling.md)).

| Worker | 위치 | 입력 | 처리 | 출력 |
|---|---|---|---|---|
| **subtitle-worker** | `apps/workers/subtitle` | `audio.mp3`, `script.json` | ffprobe 길이 측정 → 스크립트 문자 비례 SRT 생성 → 시사 키워드/숫자 하이라이트 | `subtitle.srt` → S3 |
| **render-worker** | `apps/workers/render` | `audio.mp3`, `subtitle.srt`, scenes[] | Pexels 이미지 다운로드 → zoompan 클립 → FFmpeg concat + 자막 burn-in | `output.mp4` → S3 |

> subtitle-worker는 faster-whisper를 제거하고 스크립트 텍스트 기반 SRT 생성으로 교체되었습니다.  
...
| **STT** | 스크립트 기반 SRT 생성 | faster-whisper 제거됨 — ffprobe 길이 측정 + 시사 키워드 하이라이트 |
...
│       ├── subtitle/             ← 스크립트 기반 SRT → subtitle.srt (ECS Fargate)
│       ├── render/               ← Pexels + FFmpeg → output.mp4 (ECS Fargate)
