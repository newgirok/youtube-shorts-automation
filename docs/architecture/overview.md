...
└──────┬──────────────────────────┘
       ...
       ├─── subtitle-worker (Fargate)──→ VTT/글자 비례 SRT  ──→ S3 subtitle.srt
...
| Worker | 위치 | 입력 | 처리 | 출력 |
|---|---|---|---|---|
| **subtitle-worker** | `apps/workers/subtitle` | `audio.mp3`, `subtitle.vtt`(선택), `script.json` | VTT 기반 SRT 생성 (vtt 없으면 ffprobe 오디오 길이 측정 후 글자 비례 fallback), 20자 이하 청크 분할 | `subtitle.srt` → S3 |
...
│       ├── subtitle/             ← VTT 기반 SRT → subtitle.srt (ECS Fargate)
