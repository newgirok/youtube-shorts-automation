...
└─────────────────────────────────────────────────────────┘
         ...
         ▼
┌─────────────────────────────────────────────────────────┐
│  4단계: render-worker (Lambda Container Image)           │
│                                                         │
│  SQS render-queue 수신 (Event Source Mapping)            │
...
| 항목 | 내용 |
|---|---|
| **실행 환경** | AWS Lambda Container Image (3008MB, 600s) |
| **SQS 큐** | `render-queue` |
...
| subtitle-worker | Lambda | 120초 | 600초 |
| render-worker | Lambda Container Image | 600초 | 1,200초 |
| upload-worker | Lambda | 300초 | 600초 |

**Visibility Timeout = Worker 타임아웃 × 2**: 처리 중인 메시지가 다른 Consumer에게 재배달되는 것을 방지합니다. SQS Event Source Mapping이 각 Worker를 자동으로 트리거합니다.
