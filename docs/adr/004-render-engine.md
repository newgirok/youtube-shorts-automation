# ADR 004: 렌더링 엔진 — FFmpeg (→ Phase 4: Remotion)

**상태:** Accepted (Phase 4에서 재검토)
...
- 단순 자막 오버레이는 FFmpeg `subtitles` 필터로 충분
- 빠른 파이프라인 검증 가능

**Phase 4:** Remotion으로 전환
...
- Phase 1~3 render-worker는 FFmpeg만 의존 — 이미지 경량
- Phase 4 전환 시 render-worker 내부만 교체, S3 출력 키(`jobs/{jobId}/output.mp4`)는 동일 유지
