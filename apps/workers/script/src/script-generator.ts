...
- script 글자 수 210~260자 절대 엄수 — 섹션별 목표: [기] 80~100자 / [승] 40~55자 / [전+결] 70~90자. 합산이 260자를 넘으면 [기] 팩트를 1~2개 삭제하거나 [전] 문장을 1개로 줄여 260자 이하로 맞출 것. 260자 초과 스크립트는 시스템이 자동 거부 후 재생성을 요청하므로 반드시 준수할 것
- title은 이미 벌어진 구체적 상황을 묘사해야 함 (가능성·우려 표현 금지)

레퍼런스 스크립트 예시 — 톤·구조·호흡·종결어 패턴만 학습할 것. 이 예시 자체는 글자 수가 260자를 초과하므로 길이 기준으로 삼지 말 것:
'현재 거의 대한민국 전체와 맞짱 뜨고 있다는 삼성전자 노조 — 지금 이재용 회장의 머리를 숙이게 만들 정도로 그 위세가 아주 대단하다고 하는데, 심지어 이번에 창사 이래 첫 총파업을 예고하며 반도체 라인 전체를 멈춰 버리겠다면서 지금 대한민국 경제를 인질삼아 자신들의 요구를 관철하려 삼성을 거세게 압박하고 있었다고 함. 결국 보다못한 이재용 회장이 직접 머리 숙여 대국민 사과문을 발표했는데 그걸 본 대한민국 국민들과 주주들이 지금 분통이 터지는 상황이라고. 하지만 이재용 회장의 머리를 단번에 숙이게 만들 정도로 강성했던 노조의 동력이 지금 슬슬 꺾이기 시작한 상황이라고 하는데 여러분은 누가 문제라고 봅니까?'
※ 위 레퍼런스에서 [전]+[결] 마지막 패턴을 주목할 것: '~상황이라고 하는데 여러분은 누가 문제라고 봅니까?' — '하는데' 바로 뒤에 쉼표 없이 공백 하나만 두고 comment_bait가 이어짐.
...
function parseOutput(text: string): ScriptOutput {
  ...
  if (parsed.script && parsed.script.length > 280) {
