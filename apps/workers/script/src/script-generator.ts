...
- 간접 인용 종결어 규칙 ('~합니다'·'~습니다'·'~었습니다'·'~했습니다'·'~해요'·'~하죠'·'~이죠'·'~입니다' 종결 절대 금지. 첫 문장 포함 스크립트 전체에 해당):
  ...
  [기]: 팩트 서술 문장은 종결어 없이 수치·날짜를 직접 나열하거나, '~라고 함' 최대 1회만 사용. '~이라고/~상황이라고/~거라고' 계열 사용 금지 — [기]에서 이 계열을 쓰면 [승]과 연속이 돼 규칙 위반이 됨. 매 절마다 종결어 붙이지 말 것.
  ...
  틀린 예: '~막히는 상황이 펼쳐질 거라고. ~말도 안 되는 상황이라고.' (2연속 ×) / '~초비상인 상황이라고. ~총력전 상황이라고. ~초접전 상황이라고.' (3연속 ×) / '~잠적한 상황이라고 함. ~꺾이기 시작한 상황이라고 함. 여러분은 ~?' (2연속 ×)
